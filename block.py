...
class Block:
    ...
    def repr(self):
        return f"Block({self.index}, {self.previous_hash}, {self.timestamp}, {self.data}, {self.nonce})"
...
class Blockchain:
    ...
    # Note: There might be additional methods like add_block, is_chain_valid, etc., 
    # but they are not visible in the provided images.

# Usage example (not shown in images, but typically included):
# blockchain = Blockchain()
# blockchain.add_block(Block(1, blockchain.get_latest_block().hash, time.time(), "Transaction Data"))
